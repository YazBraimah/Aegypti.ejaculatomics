...
plotGeneCPM<-function(object, gene, logMode=FALSE){
    if (grepl("MSTRG", gene)){
    ...
    p <- ggplot(subset(object, gene_id == gene), aes(sample, CPM, fill = Tissue)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=CPM-se, ymax=CPM+se), width=.2, position=position_dodge(.9)) + facet_grid(~Source, scales="free_x", space = "free_x") + labs(title = paste(gene," (", geneName,")\n",annot, sep = "")) + theme(axis.text.x=element_text(angle = 45, hjust = 1))
    if (logMode)
    {
        p <- p + scale_y_log10()
    }
